...
class Yer_istasyonu(QWidget): #ana kodun olduğu yer
    ...
    def initUI(self):
        ...
        pixmap = QPixmap('logo222.png')  # Replace with your image path
        ...
        # grafikleri oluştur
        self.sicaklikGrafik = Grafikler(self, title="sicaklik graf", xlabel="zaman", ylabel="sicaklik",     x=10, y=210, width=320, height=220)
        self.basincGrafik = Grafikler(self, title="basinc graf", xlabel="zaman", ylabel="basinc",           x=320, y=210, width=320, height=220)
        self.hizGrafik = Grafikler(self, title="hiz graf", xlabel="zaman", ylabel="hiz",                    x=630, y=210, width=320, height=220)
    ...
    def save_to_csv(self, data):
        try:
            ...
            if len(values) >= 21:
                ...
                values = [value if self.is_float(value) else '0' for value in values]
                values[3] = date_time  # Date ve Time'ı birleştirip yeni DateTime sütununa yerleştir
    ...
    def update_error_code(self, error_code):
        ...
        for i, char in enumerate(error_code):
            if i < len(self.squares):
                if char == '1':
                    self.squares[i].set_color(QColor('red'))
                else:
                    self.squares[i].set_color(QColor('green'))
